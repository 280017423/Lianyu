 ...
 * @author zou.sq
 */
public class TrendsDetailActivity extends TraineeBaseActivity implements OnClickListener {
	...
	protected void showData(ActionResult result) {
		...
		int tempfollow = mCurrentInfoModel.getIsfollow();
		if (tempfollow == 0) {
			mIbtnRelation.setImageResource(R.drawable.btn_trends_detail_no_attention_bg);
		} else {
			mIbtnRelation.setImageResource(R.drawable.btn_trends_detail_attentioned_bg);
		}
		...
		mViewChat.setTag(mCurrentInfoModel.getChatstatus());
		if (1 == mCurrentInfoModel.getChatstatus()) {
				...
				this, mCommentModelListModels, mImageLoader, new OnClickListener() {
					...
					public void onClick(View v) {
						...
						if (UserMgr.isMineUid(uid)) {
							...
							Intent intent = new Intent(TrendsDetailActivity.this, OtherHomeActivity.class);
	...
	private void initViews() {
		...
		mLvComment.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (UserMgr.isMineUid(model.getUid())) {
				...
				} else if (View.VISIBLE == mViewInput.getVisibility()) { // 点击隐藏评论输入框及软键盘
					UIUtil.setViewGone(mViewInput);
					UIUtil.setViewVisible(mViewTab);
					ImeUtil.hideSoftInput(TrendsDetailActivity.this);
				} else {
					...
					ImeUtil.showSoftInput(TrendsDetailActivity.this);
				}

			}
		});

		mSvNomal.setOnTouchListener(new OnTouchListener() {
	...
	private void getCommentLists(boolean showLoading, final boolean isAuto) {
		...
		new ActionProcessor().startAction(this, true, true, new IActionListener() {
			...
			public void onSuccess(ActionResult result) {
				...
				final List<CommentModel> commentModels = (List<CommentModel>) result.ResultObject;
	...
	private void onFinishLoad() {
		...
		new CountDownTimer(millisInFuture, millisInFuture) {
			...
			public void onFinish() {
				...
				if (UIUtil.getScreenHeight(TrendsDetailActivity.this) > mViewComment.getBottom()
						+ UIUtil.dip2px(TrendsDetailActivity.this, 45 * 2)) {
					// 重新计算高度，要是没有填充一屏的就填充满屏幕
					UIUtil.setViewHeight(
							mIvEmpty,
							UIUtil.getScreenHeight(TrendsDetailActivity.this) - mViewComment.getBottom()
									- UIUtil.dip2px(TrendsDetailActivity.this, 45 * 2));
				}
			}
		}.start();
	...
	public void onClick(View view) {
		switch (view.getId()) {
			...
			case R.id.iv_head_img:
				...
				Intent intent = new Intent(TrendsDetailActivity.this, OtherHomeActivity.class);
				intent.putExtra("touid", mCurrentInfoModel.getUid());
				startActivity(intent);
				break;
			...
			case R.id.rl_trend_detail_comment:
				...
				break;
			case R.id.ibtn_relation:
				if (null == mCurrentInfoModel) {
					return;
				}
				if (0 == mCurrentInfoModel.getIsfollow()) {
					follow("1", mCurrentInfoModel.getUid());
				} else {
					follow("2", mCurrentInfoModel.getUid());
				}
				break;
			case R.id.rl_trend_detail_chat:
				if (null == mCurrentInfoModel) {
					return;
				}
				int status = mCurrentInfoModel.getChatstatus();
				if (2 == status) {
					String userId = mCurrentInfoModel.getUid();
					String title = mCurrentInfoModel.getNickname();
					RongIM.getInstance().startPrivateChat(TrendsDetailActivity.this, userId, title);
				} else if (3 == status) {
					Intent intent1 = new Intent(TrendsDetailActivity.this, BuyServiceActivity.class);
					intent1.putExtra("touid", mCurrentInfoModel.getUid());
					intent1.putExtra("nickname", mCurrentInfoModel.getNickname());
	...
	private void follow(final String type, final String followuid) {
		new ActionProcessor().startAction(this, true, true, new IActionListener() {
			...
			public void onSuccess(ActionResult result) {
				int tempfollow = mCurrentInfoModel.getIsfollow();
				if (tempfollow == 0) {
					mCurrentInfoModel.setIsfollow(1);
					mIbtnRelation.setImageResource(R.drawable.btn_trends_detail_attentioned_bg);
				} else {
					mCurrentInfoModel.setIsfollow(0);
					mIbtnRelation.setImageResource(R.drawable.btn_trends_detail_no_attention_bg);
				}
			}
	...
	private void addComment(final String tid, final String content, final String comuid) {
		...
		new ActionProcessor().startAction(this, true, true, new IActionListener() {
			...
			public void onSuccess(ActionResult result) {
				...
				toast("评论成功");
				ImeUtil.hideSoftInput(TrendsDetailActivity.this, mEdtComment);
				UIUtil.setViewGone(mViewInput);
				UIUtil.setViewVisible(mViewTab);
